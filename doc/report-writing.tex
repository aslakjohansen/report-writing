...
\newcommand{\idx}[1]{
  \index{#1}\marginnote{\tiny #1}[0cm]
}
\newcommand{\idxx}[2]{
  \index{#1}\marginnote{\tiny #2}[0cm]
}
...
% vs goals
In projects that are explorative in nature, it often makes sense to think of requirements as \quoted{goals}. That is, unlike a typical requirement, failing to achieve it does represent a failure to reach the project goals. Instead, it is something that we strive to reach, and it is a project goal to explore to which degree we can do so. As the number of such goals increase so does the multi-dimensionality of the space that has to be explored, and the resources needed to do so.

\section{MoSCoW Method}

% brief description

% lack of analytical rationale for ranking
