...
\newcommand{\idx}[1]{\index{#1}\marginnote{\tiny #1}[0cm]}
\newcommand{\idxx}[2]{\index{#1}\marginnote{\tiny #2}[0cm]}
%\newcommand{\aslakindex}[2]{}
...
In and of itself, MoSCoW\idx{MoSCoW} is simply a categorical ranking scheme. It does not provide an analytical framework for coming up with a rationale for such a ranking. This does not mean that no rationale is needed. You should provide that rationale as a textual argument.
...
The actor-model\cite{10.5555/889486} is a software modeling approach that can be seen as a extreme foundation for the scenario-based strategy. Code is split into actors that expose services through a per-actor mailbox. Supervision trees can be used to ensure that actors keep running. This -- at least theoretically -- allows us to make per-actor scenarios and unit test for availability. This, of course, requires tooling like what is provided by the Open Telecom Platform\idx{OTP} (OTP) and -- to a lesser degree -- Kubernetes\idx{Kurbernetes}.
...
% intro: type of experimental evaluation
...
\idx{Defense} The allocated time for the defense can be seen in figure \ref{fig:defense:time}\footnote{Please note that this may not be up to date.}.
