...
\begin{figure}
  ...
  \scalebox{1.0}{
    ...
    ﻿﻿  
      ...
      % article
      \node[node] (current)  at ([xshift=1.4cm]endCondition.east) {$A_{cand}$};
      ...
      % filtering
      \node[condition] (filterCondition) at ([xshift=1.4cm]current.east) {filter};
      \draw[arrow] (current)
                -- (filterCondition);
      
      % filtered article
      \node[node] (filtered)  at ([xshift=1.4cm]filterCondition.east) {$A_{rel}$};
      ...
      \coordinate (followerAnchor) at ([yshift=-\qH/2] $([xshift=-1mm]queueAnchor)!0.5!(filtered)$);
