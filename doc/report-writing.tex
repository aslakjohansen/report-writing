...
% flushing to disk: the different levels involved
...
% proposed process
So, we need to remove the human from the loop. Luckily, with a bit of scripting skill, this is quite easy to automate. We just have to open the file, unmarshall the JSON structure, iterate through the cells, one row at a time, produce the \LaTeX code to represent it, store that code to a file, and then include that file. And in the process, while we have access to the data, we might as well enrich the representation a bit. In figure \ref{fig:eval:collection:tabular:py} you will find a full Python script that does this. It finds the minimum and the maximum cell value, and uses this to produce a heatmap effect by coloring the background of the individual cell. The resulting figure can be seen in figure \ref{fig:eval:collection:tabular:tex}. The code -- and build process -- for producing this result can be found in the GIT repository of this document (see section \ref{sec:source}).
