...
This approach translates well into a setup where experiment data across multiple runs are appended to a file. Simply add a new header before each experiment, and make sure you process the header comments as you go through the file. Then the most recent header will always be used to translate the file. An example of how this could be accomplished can be found in figure \ref{fig:eval:collection:future:header:py}. This could trivially be extended to parse cell values into integers and floats.

\begin{figure}[tbp]
  \centering
  \inputminted[fontsize=\normalsize,firstline=7,lastline=37]{python}{../src/csvheader.py}
