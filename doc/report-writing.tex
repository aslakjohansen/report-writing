...
% intro: experiment with two factors, what (some quantity that depends on two parameters, numbers that can be placed in a grid)
Tabular data fits neatly into a table of two dimensions. That is, some quantity (usually a result) depends on two parameters. This could be the result of an experiment with two factors. The resulting data would fit neatly into a CSV file, but could also be stored as JSON like that in figure \ref{fig:eval:collection:tabular:json}.
