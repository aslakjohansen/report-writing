...
\scalebox{0}{
  ...
  \textcolor{white}{\index{Exclusion criteria|see {Criteria, Exclusion}}}
  \textcolor{white}{\index{Folder|see {Directory}}}
  \textcolor{white}{\index{I|see {First person pronouns}}}
  \textcolor{white}{\index{Inclusion criteria|see {Criteria, Inclusion}}}
  ...
  \textcolor{white}{\index{Rejection criteria|see {Criteria, Exclusion}}}
  \textcolor{white}{\index{Requirement!Absolute|see {Unbounded}}}
  \textcolor{white}{\index{SE|see {Software engineering}}}
  \textcolor{white}{\index{Selection criteria|see {Criteria, Selection}}}
...
\begin{inspiration}{The Fellowship of the Ring\cite{fellowship}}
  \quoted{I wish it need not have happened in my time,} said Frodo. \quoted{So do I,} said Gandalf, \quoted{and so do all who live to see such times. But that is not for them to decide. All we have to decide is what to do with the time that is given us.}
\end{inspiration}
...
% the 40 ects trap: it ts common to be delayed after the first semester, why is that (other courses have more concrete deadlines),  still a 40ects project, effort split into conscious and subconscious, a deficiency of conscious effort can be overcome by increasing the effort (5ects can be split over a semester by working 1/6 harder), a subconscious deficiency can only be countered by adding time, if you find yourself in this situation then you have lost the main benefit of going 40 ects, an extension can't fix this as that means that you have to do additional work
...
% problem: large number of matches, not enough time, have to dwindle down
Most searches will en up with lots of candidate articles, and there simply isn't enough time to read and process every one of them. Often, it will be a few orders of magnitude off what can reasonably be considered feasible. And so, that list has to be dwindled down. We do that by filtering the list.
...
% statement and enumeration of the goals of the literature review
...
% analysis of goals leads to a n-to-m mapping to abstract queries

\subsubsection{Venues}

% mapping from queries to relevant venues: one paragraph per query
...
\textbf{Note:} This is when I realized how truly awful the state of search engines for scientific literature is. We get pretty much nothing that would allow us to implement this function. The best we can do is probably to rely on the undocumented ordering provided by them.
...
% intro: subjectivity of picking articles, reproducibility, limiting subjectivity by being up-front about criteria, inclusion criteria, rejection criteria,  inclusion vs rejection importance
When picking which articles to look into, it is easy to accidentally introduce subjectivity\idx{Subjectivity}. The temptation to pick on a whim must be resisted so that our results can be reproducible\idx{Reproducibility}. We limit subjectivity by being up-front about the criteria by which we choose articles. Such criteria are typically split into an inclusion\idxx{Criteria!Inclusion}{Inclusion criteria} criteria and an exclusion\idxx{Criteria!Exclusion}{Exclusion criteria} (or rejection) criteria. These criteria must be documented, and so must how their individual application.

% seeing criteria in terms of set theory
One way of describing the selection\idxx{Criteria!Selection}{Selection criteria} criteria is through operations on sets of articles. The inclusion criteria could be expressed as the set of articles in $\mathbb{P}_1 \cap \mathbb{P}_2 \cap (\mathbb{P}_3 \cup \mathbb{P}_4)$, where $\mathbb{P}_1$ through $\mathbb{P}_4$ are satisfied properties. If the set of articles satisfying the inclusion criteria is $\mathbb{I}$ and the set of articles satisfying the exclusion criteria (expressed in a similar way) is $\mathbb{E}$, then the set of articles chosen for study is $\mathbb{I} \backslash \mathbb{E}$.

% multistage filtering: title, abstract, full paper
In practice, it is not efficient to real a full article in order to decide on whether to include it or not. Instead, we typically to this in stages. First, we look at something that holds the potential to quickly reject it. Then we look into something that has a higher cost-per-rejection-potential, and so on. At the end we read the whole article. A common order is to first look at the title, then the abstract, and finally the whole article. However, there could also be some structure to the last step. All of this needs to be documented.

\subsubsection{Related Article Discovery}

% one paragraph of each of the methods, covering why this method is used.
