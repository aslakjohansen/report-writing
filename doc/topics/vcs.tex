...
\begin{figure}[tbp]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \coordinate (origo) at (0mm,-\spacing-1.5*\statesize);
    ...
    \tikzstyle{group}  = [
      ...
      anchor=center,
      minimum width=2*\spacing+2*\statesize,
      minimum height=2*\spacing+2*\statesize,
      scale=1.0,
    ]
    \tikzstyle{arrow}  = [
      ->,
    ...
    \node[group, anchor=center] (group) at ([xshift=2.5*\spacing+2*\statesize] origo) {};
    \node[state] (untracked) at (origo) {untracked};
    \node[state, anchor=south west] (staged) at ([xshift=2*\spacing, yshift=\spacing/2] untracked.east) {staged};
    \node[state, anchor=west] (comitted) at ([xshift=\spacing] staged.east) {comitted};
    \node[state, anchor=north] (modified) at ([yshift=-\spacing] $(staged.south)!0.5!(comitted.south)$) {modified};
    ...
    \draw[arrow,draw=purple,text=purple] ([yshift=-\statesize] group.west) -- (untracked) node[midway,sloped,above] {remove};
  \end{tikzpicture}
  \vspace{110mm}
