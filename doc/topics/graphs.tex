...
A path that has the same node as source and destination is called a \textsl{circle}\idx{Circle}, and we say that a graph that has such a path \textsl{has a} cycle. An example of such a cycle can be seen in figure \ref{fig:topics:graphs:example:cycle}.
